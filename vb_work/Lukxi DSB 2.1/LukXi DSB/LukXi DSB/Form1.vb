Public Class Form1
    ...
    Dim adress As New Adresslist
    Dim met As New WorkingClass

    Dim sc As New ScrollText(spacerScroll, heightScroll)
    Dim timetabele As New TimetabelText(vertTimetable, spacerTimetable)
    Dim info As New InfoText(vertInfo, spacerInfo)


    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load

        Dim tempA() As String = loading.TxtToSimpleArray("startupLoc.txt")
        dSrc = tempA(0)
        addrMask = tempA(1)
        addCptbr = tempA(2)
        ...
        Dim load As Boolean = loading.getData(dSrc, root)

        If load = False Then
            'MessageBox.Show("Ladefehler", "Err", MessageBoxButtons.OK)

            TimProblem.Enabled = True
    ...
    Private Sub TimTime_Tick(sender As Object, e As EventArgs) Handles TimTime.Tick
        ...
        'Beendigungstimer
        If config(10) Then
            Dim theTime As DateTime = TimeOfDay

            If DateTime.Compare(theTime, config(8)) > 0 And DateTime.Compare(theTime, config(9)) < 0 Then
                met.CloseSerial()
                If config(11) Then
    ...
    Private Sub checkRestart()
        Dim line As String = loading.TxtToString("today.txt")
        Dim today As String = Date.Today.ToString

        If line <> today Then
            restart = True
            met.StringToTxt("today.txt", today)
        End If
    End Sub

    Private Sub TimProblem_Tick(sender As Object, e As EventArgs) Handles TimProblem.Tick
        Application.Restart()
    End Sub
End Class
